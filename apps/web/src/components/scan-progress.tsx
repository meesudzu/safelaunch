    ...
    >
      <header className="border-b border-rule bg-bg/95 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 md:px-16">
          <a href={`/${locale}`} className="flex items-center gap-2 font-serif text-xl font-bold text-accent">
          ...
          <div className="flex items-center gap-3"><span className="font-mono text-xs uppercase tracking-widest text-ink-soft">{locale === "vi" ? "Đang phân tích" : "Analysis running"}</span><ThemeToggle locale={locale} /></div>
        </div>
      </header>
      ...
      <div className="mx-auto grid max-w-7xl gap-8 px-5 py-12 md:px-16 md:py-20 lg:grid-cols-12">
        ...
        <aside className="self-start lg:col-span-4 lg:col-start-9 lg:mt-[7.25rem]">
          ...
          <div className="flex items-center justify-between border-b border-rule bg-surface/50 p-4 font-mono text-xs uppercase tracking-wider text-ink-soft">
            <span>{locale === "vi" ? "Trạng thái hệ thống" : "System status"}</span><span className="text-accent">● LIVE</span>
          </div>
          ...
          <div className="p-5">
            ...
            <div className="mt-3 flex justify-between font-mono text-xs text-ink-soft"><span>{locale === "vi" ? "Tiến độ" : "Progress"}</span><span>{progressPercent}%</span></div>
          </div>
          </div>
