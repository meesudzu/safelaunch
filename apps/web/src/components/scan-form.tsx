...
export const ScanForm = ({ locale, messages, createScan }: ScanFormProps) => {
  ...
  return (
    <section aria-labelledby="headline" data-locale={locale} className="flex min-h-screen flex-col bg-bg text-ink">
      <header className="sticky top-0 z-50 border-b border-rule bg-bg/95 backdrop-blur">
        ...
        >
          ...
          <div className="flex items-center gap-3">
            ...
            <a href={`/${locale === "vi" ? "en" : "vi"}`} className="px-2 py-2 font-mono text-xs font-semibold uppercase tracking-widest text-ink-soft hover:text-accent focus-visible:outline focus-visible:outline-2 focus-visible:outline-accent">
              {messages["locale.switch"]}
            </a>
            <a href="#scan-form" className="hidden border border-accent bg-accent px-5 py-2 text-xs font-bold uppercase tracking-widest text-[#003737] hover:bg-transparent hover:text-accent sm:block">
      ...
      <div className="hero-pattern flex-1 border-b border-rule">
        ...
        <div className="flex flex-col justify-center gap-7 lg:col-span-6 lg:gap-5">
          ...
          <h1 id="headline" className="font-serif text-4xl font-extrabold leading-tight tracking-tight md:text-5xl">
        ...
        <aside aria-label={isVi ? "Báo cáo mẫu" : "Sample report"} className="relative lg:col-span-5 lg:col-start-8">
          ...
          <div className="relative flex h-full flex-col border border-rule bg-bg">
            <div className="flex items-center justify-between border-b border-rule bg-surface/50 p-4 font-mono text-xs uppercase tracking-wider text-ink-soft">
              <span>{isVi ? "Báo cáo mẫu" : "Sample report"}</span><span>•••</span>
            </div>
            <div className="flex flex-1 flex-col gap-6 p-5 md:p-6 lg:gap-4 lg:p-4">
              <div className="flex items-start justify-between gap-4">
                <div><h2 className="font-serif text-xl font-bold">Example.vn</h2><p className="mt-1 text-sm text-ink-soft">{isVi ? "Quét hoàn tất lúc 14:32" : "Scan completed at 14:32"}</p></div>
                <span className="border border-error px-3 py-1 font-mono text-xs font-bold uppercase text-error">{isVi ? "Cần xem xét" : "Review"}</span>
              </div>
              <div className="grid grid-cols-3 border-y border-rule py-4 lg:py-3">
                {[["2", isVi ? "Rủi ro cao" : "High risk", "text-error"], ["6", isVi ? "Cảnh báo" : "Warnings", "text-gold"], ["14", isVi ? "Đạt chuẩn" : "Passed", "text-accent"]].map(([value, label, color], index) => (
                  <div key={label} className={`flex flex-col gap-1 ${index ? "border-l border-rule pl-4" : ""}`}>
                    <strong className={`font-serif text-4xl ${color}`}>{value}</strong><span className="font-mono text-[10px] uppercase tracking-wider text-ink-soft">{label}</span>
                  </div>
                ))}
              </div>
              <div className="space-y-1 text-sm">
                <p className="flex justify-between border-b border-rule py-3 lg:py-2"><span><b className="mr-2 text-error">×</b>{isVi ? "Thiếu chính sách bảo mật" : "Missing privacy policy"}</span><span className="font-mono text-[10px] text-error">CRITICAL</span></p>
                <p className="flex justify-between border-b border-rule py-3 lg:py-2"><span><b className="mr-2 text-gold">!</b>{isVi ? "Thông tin giấy phép chưa đủ" : "License details incomplete"}</span><span className="font-mono text-[10px] text-gold">WARN</span></p>
                <p className="flex justify-between py-3 lg:py-2"><span><b className="mr-2 text-accent">✓</b>{isVi ? "Kết nối HTTPS hợp lệ" : "HTTPS connection valid"}</span><span className="font-mono text-[10px] text-accent">PASS</span></p>
              </div>
              <div className="mt-auto"><div className="h-1 bg-rule"><div className="h-full w-[63%] bg-accent" /></div><div className="mt-2 flex justify-between font-mono text-xs text-ink-soft"><span>{isVi ? "Mức độ đáp ứng" : "Compliance coverage"}</span><span>63/100</span></div></div>
            </div>
          </div>
        </aside>
        </div>
